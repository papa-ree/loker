<div x-data="{ 
    ...
    <x-core::breadcrumb :items="[['label' => __('Kategori Lowongan'), 'route' => 'loker.category.index']]" :active="$categoryId ? __('Edit Kategori') : __('Tambah Kategori')" />
    ...
    <div class="max-w-4xl mx-auto mt-6">
        <div class="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-xl overflow-hidden">
            <!-- Header Banner -->
            <div class="p-6 border-b border-gray-100 dark:border-slate-800 bg-linear-to-r from-purple-50/50 to-indigo-50/50 dark:from-purple-900/10 dark:to-indigo-900/10">
                <h3 class="text-lg font-bold text-gray-900 dark:text-white">
                    {{ $categoryId ? __('Edit Detail Kategori') : __('Tambah Kategori Baru') }}
                </h3>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                    {{ __('Gunakan kategori untuk mengelompokkan lowongan kerja.') }}
                </p>
            </div>

            <form wire:submit="save" class="p-8 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <x-core::input label="{{ __('Nama Kategori') }}" wire:model="name" x-model="name" required />
                    <x-core::input label="{{ __('Slug') }}" wire:model="slug" x-model="slug" x-slug="name" required />
                </div>

                <x-core::input label="{{ __('Icon Lucide') }}" wire:model="icon" placeholder="Contoh: brief-case, code, heart" />
                
                <x-core::textarea label="{{ __('Deskripsi Kategori') }}" wire:model="description" rows="4" />
                ...
                <div class="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800/30 rounded-xl border border-slate-100 dark:border-slate-800">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <label for="active-toggle" class="text-sm font-bold text-gray-900 dark:text-white cursor-pointer">{{ __('Status Aktif') }}</label>
                            <p class="text-[10px] text-gray-500 uppercase tracking-wider">
                                {{ __('Aktifkan agar kategori dapat dipilih di lowongan') }}
                            </p>
                        </div>
                    </div>
                ...
                <div class="pt-6 border-t border-gray-100 dark:border-slate-800 flex items-center justify-between">
                    <x-core::secondary-button link href="{{ route('loker.category.index') }}" label="{{ __('Batal') }}" />
                    <x-core::button type="submit" label="{{ __('Simpan Kategori') }}" spinner="save">

<div x-data="{ 
    ...
    <x-core::breadcrumb :items="[['label' => __('Manajemen Perusahaan'), 'route' => 'loker.company.index']]"
        :active="$companyId ? __('Edit Perusahaan') : __('Tambah Perusahaan')" />
    ...
    <div class="max-w-4xl mx-auto mt-6">
        <div
            ...
            <!-- Header Banner -->
            <div
                class="p-6 border-b border-gray-100 dark:border-slate-800 bg-linear-to-r from-emerald-50/50 to-teal-50/50 dark:from-emerald-900/10 dark:to-teal-900/10">
                <h3 class="text-lg font-bold text-gray-900 dark:text-white">
                    {{ $companyId ? __('Edit Profil Perusahaan') : __('Registrasi Perusahaan Baru') }}
                </h3>
                <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                    {{ __('Lengkapi data identitas dan kontak perusahaan mitra.') }}
                </p>
            </div>
            ...
            <form wire:submit="save" class="p-8 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="">
                        <x-core::input label="{{ __('Nama Perusahaan') }}" wire:model="name" x-model="name" required />
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="">
                        <x-core::input label="{{ __('Website') }}" wire:model="website" placeholder="https://..." />
                        <x-core::input-error for="website" />
                    </div>
                    <x-core::input label="{{ __('Logo URL') }}" wire:model="logo" placeholder="URL logo perusahaan" />
                </div>

                <div class="">
                    <x-core::textarea label="{{ __('Alamat') }}" wire:model="address" rows="3" />
                    <x-core::input-error for="address" />
                </div>

                <div class="">
                    <x-core::textarea label="{{ __('Deskripsi Perusahaan') }}" wire:model="description" rows="5" />
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] text-gray-500 uppercase tracking-wider">
                                {{ __('Aktifkan agar perusahaan dapat dipilih di lowongan') }}
                            </p>
                        </div>
                    </div>
                ...
                <div class="pt-6 border-t border-gray-100 dark:border-slate-800 flex items-center justify-between">
                    <x-core::secondary-button link href="{{ route('loker.company.index') }}"
                        label="{{ __('Batal') }}" />
                    <x-core::button type="submit" label="{{ __('Simpan Perusahaan') }}" spinner="save">

<tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group">
    <td class="px-6 py-4">
        <div class="flex items-center gap-3">
            <div class="p-2 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg">
                <x-lucide-building-2 class="w-5 h-5 text-emerald-600 dark:text-emerald-400" />
            </div>
            <div>
    ...
    <td class="px-6 py-4 whitespace-nowrap w-px">
        @canany(['loker.company.update', 'loker.company.delete'])
            <livewire:core.shared-components.item-actions :editUrl="route('loker.company.edit', $record->id)"
                :deleteId="$record->id" :navigate="true" wire:key="company-actions-{{ $record->id }}"
